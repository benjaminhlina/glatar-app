...
This app has the ability to upload (i.e., 
...
The repository contains the Shiny App built as a package (i.e, [{glatar}](https://benjaminhlina.github.io/glatar-app/)) which is checked by R CMD Check. A [pkgdown website](https://benjaminhlina.github.io/glatar-app/) is produced to assist in documentation of the app. Once the package has built, it triggers a GitHub Action (GHA) that builds a docker container that installs the package and copies the `app.R` file which shiny-server will run. This docker container is registred on GitHub Container Registery (GHCR). Once complete 
