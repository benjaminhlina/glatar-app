...
#   missing <- setdiff(c("common_name", "scientific_name"), names(df))
#   if (length(missing) > 0) {
#     cli::cli_abort(
#       "df is missing the following taxa column{s}: {.val {missing}}"
#     )
#   }
...
#   # Join both ways — rename the paired name in each to avoid collision
#   has_sci <- !is.na(df$scientific_name)

#   # Match on scientific_name, cross-fill common_name from taxonomy
#   by_sci <- df[has_sci, ] |>
#     dplyr::left_join(
#       species_list,
#       by = "scientific_name",
...
#     ) |>
#     dplyr::mutate(
#       common_name = dplyr::coalesce(common_name, common_name_tax)
#     ) |>
#     dplyr::select(-dplyr::ends_with("_tax"))

#   # Match on common_name, cross-fill scientific_name from taxonomy
#   by_com <- df[!has_sci, ] |>
#     dplyr::left_join(
#       species_list,
#       by = "common_name",
...
#     ) |>
#     dplyr::mutate(
#       scientific_name = dplyr::coalesce(scientific_name, scientific_name_tax)
#     ) |>
#     dplyr::select(-dplyr::ends_with("_tax"))

#   df_joined <- dplyr::bind_rows(by_sci, by_com)
